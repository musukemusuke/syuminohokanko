...
# ★ `/setup` コマンドで非表示VC金庫と部屋を丸ごと生成する正しい命令
@bot.tree.command(name="setup", description="【管理者専用】ブックマーク用のカテゴリーとチャンネルを生成します")
...
async def setup_channels(interaction: discord.Interaction):
    ...
    cat_name = "📁 ブックマーク"
    cat = discord.utils.get(guild.categories, name=cat_name) or await guild.create_category(name=cat_name)
    ...
    # 3. ★【完全非表示ロック】お相手（オーナー）の画面からも完全にHideするボイスチャンネルを生成
    ch_vc = discord.utils.get(cat.voice_channels, name="🤫・データ金庫")
    if not ch_vc:
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False, connect=False), # 一般人はHide
            guild.owner: discord.PermissionOverwrite(view_channel=False, connect=False),        # ★お相手（オーナー）も完全にHide
            guild.me: discord.PermissionOverwrite(view_channel=True, connect=True, send_messages=True) # ボットだけが見える
        }
    ...
    # 部屋の使い方の初期案内メッセージとボタンを自動設置
    await check_and_restore_messages(bot, post_id, archive_id, my_embed_factory)

    await interaction.followup.send("✅ チャンネルと秘密の金庫VCの自動生成が完了しました！", ephemeral=True)
    ...
    if bool(message.attachments) or "http" in message.content:
        ...
        async for msg in storage_vc.history(limit=1000):
            if msg.content.startswith("🆕NEW_FOLDER:"):
                ...
                if int(lines.replace("👤USER:", "").strip()) == message.author.id:
                    folders.append(lines.replace("🆕NEW_FOLDER:", "").strip())
        if not folders:
